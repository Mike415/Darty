...
function iosWebViewPlugin(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      ...
      // Find the script tag in head and move it to end of body
      // Remove type="module" and crossorigin for IIFE compatibility
      const scriptMatch = html.match(/<script[^>]*src="\.\/app\.js"[^>]*><\/script>/);

      if (scriptMatch) {
        // Remove script from wherever it is
        html = html.replace(scriptMatch[0], '');
        // Add script at end of body without type="module"
